...
# ╔═╡ 9ce44eb8-0a30-11eb-2b10-b703346cbbc6
if !@isdefined(coordinate_after_random_move)
	not_defined(:coordinate_after_random_move)
else
	let
		if coordinate_after_random_move isa Missing
			still_missing()
		elseif !(coordinate_after_random_move isa Coordinate)
			keep_working(md"Make sure that you return a `Coordinate`.")
		else
			if any(possible_moves) do m
					coordinate_after_random_move == Coordinate(3,4) + m
				end
				correct()
			else
				keep_working()
