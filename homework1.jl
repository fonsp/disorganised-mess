...
let
	...
	[extend(philip_head, i, j) for (i, j) in Iterators.product(-50:size(philip_head,1)+51, (-50:size(philip_head,2)+51))]
end
...
elseif length(Set(random_vect)) != 10
	...
	correct(md"Well done! You can run the cell above again to generate a new vector!")
end
...
# ╔═╡ 4934621e-9a9f-4dec-8950-cdd9e655745f
todo("We should include more pointers to basic Julia tutorials")

# ╔═╡ 675fc465-b87d-4e7c-a65c-1dea2e609fa2
md"""
In the `demean` exercise, we should talk about **mutation**. Can we have them _discover_ the problem with mutation instead of saying it?

## Idea 

We could have an exercise to write two visual tranformations: `flip180` and `invert`. If you show those, applied to the same image, next to eachother, the second image will be flipped **and** inverted.""" |> todo

# ╔═╡ 237f0255-13ce-4933-aabf-d08035bbe077
todo(md"Last year these next exercises proved too difficult, but we should still do something to introduce 2D arrays")

# ╔═╡ fc0b294b-3269-4ed5-8fa8-04729538cf91
todo("Let's first have them play around with this image")

# ╔═╡ d6a45284-ee41-417b-909e-d1bf3cd1963b
todo("I don't like that we are returning a tuple here instead of a color")
