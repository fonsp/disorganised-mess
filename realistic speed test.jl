...
# ╔═╡ b9c7b016-f788-4001-b8dc-2c74c53f9ef5
md"""
### Start both clocks:
and wait 6 seconds
"""

# ╔═╡ c2ceb3e3-2a96-4c16-879b-99060e1443bb
md"""Run superclock: $(@bind do_clock html"<input type=checkbox>")"""

# ╔═╡ 5223a84a-c5c8-4037-9f36-a722d9f969b9
num_ticks = Ref(0)

# ╔═╡ ea15cb51-a82e-4c0c-9b1b-95eb6e920269
Δt = 3

# ╔═╡ f6fc3b1e-7278-4d27-8b83-04ddee85a3b7
@bind reset Clock(Δt)

# ╔═╡ d44c9436-499b-48e3-83f5-2138bb778ecb
let
	reset
	freq = num_ticks[] / Δt
	num_ticks[] = 0
...
# ╔═╡ aef6fd04-cce7-4bad-aa83-76155119f0de
supertimer = html"""
<script>
let i = 0
...
# ╔═╡ 7336a989-9052-41fc-9f03-20367463a575
if do_clock === true
	@bind trigger supertimer
end

# ╔═╡ fd6abcee-7537-4ca2-809f-5089ca63750b
let
	trigger
	num_ticks[] += 1
end
...
# ╔═╡ 2e64ed1a-b766-41a5-8204-883e71169693
fonsi = 15

# ╔═╡ dd197bcc-47ef-4f03-9604-47ccb32524c4
repeat('\n',fonsi) |> Text
...
# ╔═╡ ff2cc1c7-83ef-4dbe-95f5-79f03fdd7a44
@bind kljasdfkjlsdfkjldsfkjlfdskjl Slider(1:.01:10)

# ╔═╡ faf2ec04-12c2-11eb-2162-9bfd2ee22a07
...
# ╔═╡ e2495785-c61b-4c23-8ea5-da90fcfddb2b
@bind ooo Slider(1:100000)

# ╔═╡ 4af75d92-0ffb-4316-b2ce-f35c1bf6f533
ooo
...
# ╔═╡ a9e91932-12cc-11eb-1340-b731480c6d29
...
# ╔═╡ 9a0a82d6-12df-11eb-07ab-0ffa63aff9a1
...
# ╔═╡ 6186a6ca-12e3-11eb-0168-5770be55ca7d
...
# ╔═╡ c2f66e5c-a8ce-4b7e-ada3-6939a5a6bde5
asdf = guess_μ - 1

# ╔═╡ be99e576-579a-4cde-9025-d4507b39a9a8
ppppp = let
	sqrt(asdf)
end

# ╔═╡ cf3decab-6f3e-4772-b6c7-19fa5a87a367
opop = [
	ppppp for i in 1:2
		]

# ╔═╡ 24ee0fbf-5548-4675-9828-003f5581850f
...
# ╔═╡ e80ca50e-12cb-11eb-28eb-87f3159b5467
...
# ╔═╡ 4d026026-12d5-11eb-3231-25800d0c5372
found_β, found_γ = let
...
let
	slope = wavy_deriv(kljasdfkjlsdfkjldsfkjlfdskjl)
	
	p = plot(LinRange(1.0 - 0.1, 1.0 + 0.1, 2), wavy, label=nothing, lw=3)
	scatter!(p, [1], wavy, label="f(1)", color="blue", lw=3)
	# p = plot()
	x = [kljasdfkjlsdfkjldsfkjlfdskjl - 0.2,kljasdfkjlsdfkjldsfkjlfdskjl + 0.2]
	for y in -4:10
		plot!(p, x, slope .* (x .- kljasdfkjlsdfkjldsfkjlfdskjl) .+ y, label=nothing, color="purple", opacity=.6)
	end
	
	vline!(p, [kljasdfkjlsdfkjldsfkjlfdskjl], color="red", label="a", linestyle=:dash)
