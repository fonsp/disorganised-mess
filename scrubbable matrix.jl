...
# ╔═╡ f0902306-2b26-49e8-8ca5-d00668c374ba
using SparseArrays

# ╔═╡ 5e7f5814-2ac2-4650-ae81-913f2911198d
using LinearAlgebra

# ╔═╡ 8442370e-86e1-4a2b-bf10-3640edded545
using Plots
...
# ╔═╡ cfe6e108-e97c-4271-8ff3-53aa3907a777
unwrap(input::Dict) = collect(reshape(input["values"], Tuple(input["size"]) |> reverse)')

# ╔═╡ ac9cc882-8a28-462f-ba77-f7aa24b47c04
unwrap(input::Vector{<:Real}) = throw(ArgumentError("""
Unwrapping this bind value needs a template as the first argument to know which size of matrix to create. 

Use unwrap(template_matrix, data_from_bind).
"""))

# ╔═╡ d8b7ef38-7b3b-492b-a08a-d6024af11afc
unwrap(template::AbstractMatrix{T}, input::Vector) where T = Matrix{T}(reshape(input, size(template) |> reverse)')
...
# ╔═╡ b9ae27d2-c252-4efb-8b7b-84183cab3b15
# SelectEntries(rand(100,100))
...
begin
	...
	Base.get(s::SelectEntries) = vec(s.default') |> collect
...
const sel = selection.getSelection()
		return {
			size,
			values: dots.map(el => sel.includes(el)),
		}
},
	set: val => {
		if(!val){
...
update_classes()
	...
	configurable: true,
		...
		root.value = {size, values: $(vec(s.default'))}
...
# ╔═╡ c3295d07-8cd7-4d9b-ac19-77c2a47b5668
md"""
## Combining bonds
"""

# ╔═╡ 1bfb076b-fb24-47e0-b970-84e0cf01f259
...
<script>
	...
	const inputs = div.querySelectorAll("pl-combined_child > *:first-child")
	...
	Object.defineProperty(div, 'value', {
		...
		set: (newvals) => {
			...
			inputs.forEach((el, i) => {
			values[i] = newvals[i]
			/// TODO THIS IS NOT ALWAYS HOW YOU SET THE VALUE, SEE PLUTO SOURCE CODE
			el.value = newvals[i]
...
# ╔═╡ 1ff6d2f3-f7f9-4b43-8e52-582eab92c0dc
combined_child(x) = @htl("""<pl-combined_child>$(x)</pl-combined_child>""")

# ╔═╡ 443f37a0-a22a-4d79-b44c-7666ca9be0cc
begin
	struct BondDefault
		x
		default
	...
	Base.show(io::IO, m::MIME"text/html", bd::BondDefault) = show(io, m, bd.x)
...
@bind which_entries_raw1 SelectEntries(A)

# ╔═╡ d2aaaf27-bef4-4578-9560-38f41abfd685
which_entries1 = unwrap(which_entries_raw1)

# ╔═╡ 0a386c73-5996-49f8-ba6a-257d0c360df7
@bind which_entries_raw2 SelectEntries(A)

# ╔═╡ 53f661ce-834f-49fc-b12a-df901b901cd0
which_entries2 = unwrap(which_entries_raw2)
...
function scrub_matrix(A::Matrix{<:Real}; kwargs...)
	...
	<div style=$(Dict(
		...
		map(eachindex(A')) do I
			...
			combined_child(Scrubbable(default_range(init); default=init, kwargs...))
		end
	...
	BondDefault(combine(h), vec(A') |> collect)
end
...
# ╔═╡ e4f39f02-9739-4e9b-8904-3bb645f2b59a
together
